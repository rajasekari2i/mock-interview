...
@pytest.mark.asyncio
async def test_candidate_transition_preserves_profile_and_provenance(
    db_session: AsyncSession, factories: object, clock: object
) -> None:
    organization = factories.organization()
    ...
    assert user.role == Role.MANAGER.value and user.auth_generation == 2
    ...
    await change_user_role(
        ...
        role=Role.ADMIN,
        actor_user_id=user.id,
        correlation_id="request-2-admin",
        now=clock.now(),
    )
    await change_user_role(
        db_session,
        ...
        role=Role.CANDIDATE,
        actor_user_id=user.id,
        correlation_id="request-2-candidate",
        now=clock.now(),
    )
    assert user.role == Role.CANDIDATE.value and user.auth_generation == 4
    assert user.registration_domain_mapping_id == mapping.id
